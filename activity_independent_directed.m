% Script to simulate directed activity-independent model
...
for i = 1:num_updatesBurn
    ...
    A = A - full(sparse(I(inds_remove), J(inds_remove), A(edge_inds(inds_remove)), N, N));
    ...
    A = A + full(sparse(I(inds_inc), J(inds_inc), ones(length(inds_inc), 1), N, N));
